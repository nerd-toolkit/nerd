...
//Chris
void TestBrentOneDimMinimizer::testQuadraticLineMinimization() 
{
  ...
  OneDimFunctionPoint p = blm->minimize(f);
  
  QVERIFY( fabs(p.x - f->minX) <= DELTA);
  QVERIFY( fabs(p.y - f->minY) <= DELTA);
