	...
	if(Core::getInstance()->init()) {
		...
		if(filesArgument->getParameterValue()->get() != "") {
			...
			if(!fileNames.at(0).endsWith(".onn")) {
				...
				NeuralNetwork* net = NeuralNetworkIO::createNetworkFromFile(fileNames.at(0), &errorMsg, &warnings);
			...
			cout << "NetToByteConverter: No input and output files specified!" << endl
				<< "See NetToByteConverter --help for more information." << endl;
		}
	}
