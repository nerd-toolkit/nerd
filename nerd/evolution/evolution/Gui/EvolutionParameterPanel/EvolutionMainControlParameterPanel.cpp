...
EvolutionMainControlParameterPanel::EvolutionMainControlParameterPanel()
	: mGenerationCompletedEvent(0), mShutDownEvent(0)
{
...
void EvolutionMainControlParameterPanel::addPopulationParameters(QVBoxLayout *layout) {
	...
	ValueManager *vm = Core::getInstance()->getValueManager();
	QStringList desiredPopNames = vm->getValueNamesMatchingPattern("/Evo/Pop/.*/DesiredPopulationSize");

	for(QListIterator<QString> i(desiredPopNames); i.hasNext();) {
		QString desiredPopName = i.next();
		QString name = desiredPopName.mid(QString("/Evo/Pop/").size());
...

		{
			...
			IntValue *desPopSize = vm->getIntValue("/Evo/Pop/" + name + "/DesiredPopulationSize");
			if(desPopSize != 0) {
				populationLayout->addWidget(new ParameterControlWidget("DesiredPopSize", desPopSize, 120));
			}
			IntValue *numPreservedParents = vm->getIntValue("/Evo/Pop/" + name + "/NumberOfPreservedParents");
		...
		QStringList selectionNames = vm->getValueNamesMatchingPattern(
						"/Evo/Pop/" + name + "/Selection/.*/PopulationProportion");
		for(QListIterator<QString> j(selectionNames); j.hasNext();) {
			QString selectionName = j.next();
	
			selectionName = selectionName.mid(QString("/Evo/Pop/" + name + "/Selection/").size());
			selectionName = selectionName.mid(0, selectionName.indexOf("/"));

			{
				QVBoxLayout *mainLayout = new QVBoxLayout();
				QFrame *selectionFrame = new QFrame();
				selectionFrame->setFrameStyle(QFrame::WinPanel | QFrame::Raised);
				selectionFrame->setLineWidth(1);
				selectionFrame->setContentsMargins(1, 1, 1, 1);
				QHBoxLayout *nameLayout = new QHBoxLayout();
				selectionFrame->setLayout(nameLayout);
		
				//Add Selection parameters
				QLabel *nameLabel = new QLabel("Selection: " + selectionName + " (" + name + ")");
				nameLayout->addWidget(nameLabel);
				...
				QString selectionPrefix = "/Evo/Pop/" + name + "/Selection/" + selectionName;

				Value *popProp = vm->getValue(selectionPrefix + "/PopulationProportion");
				if(popProp != 0) {
					selectionLayout->addWidget(new ParameterControlWidget("PopProportion", popProp, 120));
				}

				QStringList otherProps = vm->getValueNamesMatchingPattern(selectionPrefix + "/.*");
				for(QListIterator<QString> k(otherProps); k.hasNext();) {
					QString propName = k.next();
