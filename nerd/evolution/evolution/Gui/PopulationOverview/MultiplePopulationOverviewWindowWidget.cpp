...
MultiplePopulationOverviewWindowWidget::MultiplePopulationOverviewWindowWidget(QWidget *parent) 
	: QWidget(parent), mInitialized(false), mCurrentGenerationValue(0) 
{
...
void MultiplePopulationOverviewWindowWidget::showOverview() {
	if(!mInitialized) {
		...
		for(int i = 0; i < worlds.size(); i++) {
			QList<Population*> populations = worlds.at(i)->getPopulations();
