...
#include "ESMultiDensityFuncObjParaMutation.h"
#include "Math/Math.h"

namespace nerd {

ESMultiDensityFuncObjParaMutation::ESMultiDensityFuncObjParaMutation()
{}

QList<OptimizationDouble> ESMultiDensityFuncObjParaMutation::doMutation(
		...
		const ESInformation &information)
{
	...
	// check if for every object parameter exists mutation strength
	if(strategyParameters.size() != objectParameters.size()){
		return mutatedObjParas;
	}
	...
	for(int i = 0; i < objectParameters.size(); i++)
	{
		...
		double newValue = objectParameters.at(i).value 
											+ (strategyParameters.at(i).value * randomSample);
