...
int main(int argc, char *argv[])
{
	...
	if(Core::getInstance()->init()) {
		...
		if(filesArgument->getParameterValue()->get() != "") {
			...
			if(!fileNames.at(1).endsWith(".onn")) {
				cout << "Destination file has no *.onn ending!" << endl;
			} else {
				...
				if((net != 0) && (errorMsg.isEmpty())) {
					...
					// Write net to nerd file
					NeuralNetworkIO::createFileFromNetwork(fileNames.at(1), net, &errorMsg, &warnings);
					if(errorMsg.isEmpty()) {
						warningsIt.toFront();
						while(warningsIt.hasNext()) {
							cout << "Warning: " << warningsIt.next().toStdString() << endl;
			...
			cout << "SimbaV3XmlASeriesNetToNerdConverter: No input and output files specified!" << endl
				<< "See SimbaV3XmlASeriesNetToNerdConverter --help for more information." << endl;
		}
	}
