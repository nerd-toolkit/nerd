...
int main(int argc, char *argv[])
{
	...
	if(Core::getInstance()->init()) {
		...
		if(filesArgument->getParameterValue()->get() != "") {
			...
			// Check if sourceFileName ends with .onn
			if(!fileNames.at(0).endsWith(".onn")) {
				cout << "Source file is no Nerd Xml (*.onn) file!" << endl;
			} else {
				...
				ModularNeuralNetwork* net = dynamic_cast<ModularNeuralNetwork*>(
						NeuralNetworkIO::createNetworkFromFile(fileNames.at(0), &errorMsg, &warnings));
				if((net != 0) && (errorMsg.isEmpty())) {
					...
					// Write net to Simba ASeries file
					NeuralNetworkIOSimbaV3Xml::createASeriesFileFromNetwork(fileNames.at(1), net, &errorMsg);
					if(!errorMsg.isEmpty()) {
			...
			cout << "NerdASeriesNetToSimbaV3XmlConverter: No input and output files specified!" << endl
				<< "See NerdASeriesNetToSimbaV3XmlConverter --help for more information." << endl;
		}
	}
