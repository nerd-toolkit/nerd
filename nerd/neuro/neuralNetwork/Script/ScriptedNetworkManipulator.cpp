...
/**
 * Sets the maximal concentration that can be produced by the modulator cell of the target network element.
 */
void ScriptedNetworkManipulator::setMaxConcentration(qulonglong elementId, int type, double maxConcentration) {
	if(mNetwork == 0) {
		return;
	}
	...
	if(modulator != 0) {
		modulator->setMaxConcentration(type, maxConcentration);
	}
}


/**
 * Returns the maximal concentration that can be generated by the modulator cell of the target network element.
 */
double ScriptedNetworkManipulator::getMaxConcentration(qulonglong elementId, int type) {
	if(mNetwork == 0) {
		return 0.0;
	}
	...
	NeuroModulator *modulator = getNeuroModulator(elementId, &object);
	if(modulator != 0) {
		return modulator->getMaxConcentration(type);
	}
	return 0.0;
}
...
/**
 * Returns an array that contains the internal variables of all parameters for the current modulator type
 * that is active (used by the current update modus) for a given network element.
 * If the network element is not providing a NeuroModulator object, then an empty array is returned.
 * 
 * To determin which parameter (index in the array) is used for which purpose, use the
 * getModulatorUpdateVariableNames() function. The order of objects in both arrays is the same, so 
 * the name array can be used as a lookup table to get the variable names of the internal variables.
 */
...
/**
 * Returns an array that contains the names of all parameters for the current modulator type
 * that is active (used by the current update modus) for a given network element.
 * If the network element is not providing a NeuroModulator object, then an empty array is returned.
 */
...
/**
 * Returns the NeuroModulator object (modulator cell) that is associated with the network element 
 * specified with the elementId.
 * 
 * If the target element does not provide a NeuroModulator object, then 0 is returned.
 */
