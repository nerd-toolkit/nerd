...
#include "MSeriesV2Model_Obsolete.h"
#include <iostream>
#include <QList>
#include "Core/Core.h"
#include "Core/Core.h"
...
/**
 * Constructs a new MSeriesV2Model_Obsolete.
 */
MSeriesV2Model_Obsolete::MSeriesV2Model_Obsolete(const QString &groupName, const QString &bodyPartList) 
 ...
 * Copy constructor. 
 * 
 * @param other the MSeriesV2Model_Obsolete object to copy.
 */
MSeriesV2Model_Obsolete::MSeriesV2Model_Obsolete(const MSeriesV2Model_Obsolete &other) : Object(), ValueChangedListener(), 
 ...
 * Destructor.
 */
MSeriesV2Model_Obsolete::~MSeriesV2Model_Obsolete() {
}

SimObject* MSeriesV2Model_Obsolete::createCopy() const {
	MSeriesV2Model_Obsolete *copy = new MSeriesV2Model_Obsolete("Default", mBodyPartSelectorValue->get());
...
void MSeriesV2Model_Obsolete::createModel() {
	...
	if(mIncludeLeftArm) {
		...
		if(mUseAlternativeLeftArm) {
			PARAM(DoubleValue, mLeftElbowPitchMotor, "MinAngle")->set(-20);
			PARAM(DoubleValue, mLeftElbowPitchMotor, "MaxAngle")->set(120);
		}
		else {
			PARAM(DoubleValue, mLeftElbowPitchMotor, "MinAngle")->set(0);
			PARAM(DoubleValue, mLeftElbowPitchMotor, "MaxAngle")->set(105);
		}
...
void MSeriesV2Model_Obsolete::layoutObjects() {
...
// 		InterfaceValue *motorRollMotorAngle = dynamic_cast<InterfaceValue*>(mHeadRollMotor->getParameter("JointAngle"));
// 		motorRollMotorAngle->setMin(-45);
// 		motorRollMotorAngle->setMax(45);
	}
	...
	if(mLeftElbowPitchMotor != 0) {
		if(mUseAlternativeLeftArm) {
			PARAM(Vector3DValue, mLeftElbowPitchMotor, "AxisPoint1")->set(leftShoulderRollX, elbowY, mElbowJointZ->get());
			PARAM(Vector3DValue, mLeftElbowPitchMotor, "AxisPoint2")->set(leftShoulderRollX, elbowY, mElbowJointZ->get() + 0.1);
		}
		else {
 ...
 */
void MSeriesV2Model_Obsolete::disableCollisions(SimBody *body1, SimBody *body2) {
	if(body1 != 0 && body2 != 0) {
...
void MSeriesV2Model_Obsolete::initializeMotorModels() {
