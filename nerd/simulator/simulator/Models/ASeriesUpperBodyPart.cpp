...
#include "ASeriesUpperBodyPart.h"
#include <iostream>
#include <QList>
#include "Core/Core.h"
...
/**
 * Constructs a new ASeriesUpperBodyPart.
 */
ASeriesUpperBodyPart::ASeriesUpperBodyPart(const QString &groupName) : ModelInterface(groupName) {
 ...
 * Copy constructor. 
 * 
 * @param other the ASeriesUpperBodyPart object to copy.
 */
ASeriesUpperBodyPart::ASeriesUpperBodyPart(const ASeriesUpperBodyPart &other) 
 ...
 * Destructor.
 */
ASeriesUpperBodyPart::~ASeriesUpperBodyPart() {
}

void ASeriesUpperBodyPart::createModel() {
...
// 	agent->addObject(leftFootLR);
	...
	Vector3DValue *pos = 0;
	QuaternionValue *orientation = 0;

// 	SimObject *abfr = accelSensor->createCopy();
// 	abfr->setName(mNamePrefix + ("ABFR"));
// 	pos = dynamic_cast<Vector3DValue*>(abfr->getParameter("LocalPosition"));
// 	pos->set(accelOffsetFootX, accelOffsetFootY, accelOffsetFootZ);
// 	orientation = dynamic_cast<QuaternionValue*>(abfr->getParameter("LocalOrientation"));
...
SimObject* ASeriesUpperBodyPart::createCopy() const {
	ASeriesUpperBodyPart *copy = new ASeriesUpperBodyPart("Default");
	return copy;
}
